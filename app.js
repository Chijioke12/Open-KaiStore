...
var REGISTRY_PAGES_BASE = 'https://chijioke12.github.io/Open-KaiStore-Registry';
	  ...
	  installApp: function installApp(app) {
	    ...
	    console.log('Installing:', app.name);
	        ...
	        if (app.type === 'packaged') {
	          var manifestUrl = app.manifest_url || (app.id ? "".concat(REGISTRY_PAGES_BASE, "/manifests/").concat(app.id, ".json") : '');
	          ...
	          } else if (manifestUrl && navigator.mozApps.install && typeof navigator.mozApps.install === 'function') {
	            // Install packaged apps via a "mini manifest" that contains `package_path`.
	            request = navigator.mozApps.install(manifestUrl);
	          } else {
	            throw new Error('This device does not support packaged installs via installPackage(). Enable GitHub Pages for the registry and ensure a mini-manifest exists, or install via OmniSD/File Manager.');
	          }
	        } else {
